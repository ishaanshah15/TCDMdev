...
class PointCloudGenerator(object):
    ...
    @param max_bound: If not None, list len(3) containing largest x, y, and z
        values that will not be cropped
    """
    def __init__(self, sim, savepath, min_bound=None, max_bound=None):
        ...
        self.cam_names = [1] #np.arange(self.sim.model.ncam)

        print('camera_indices',self.cam_names)
    ...
    def generateCroppedPointCloud(self):
        o3d_clouds = []
        cam_poses = []
        for cam_index in range(len(self.cam_names)):
            cam_i = self.cam_names[cam_index]
            ...
            np.save(self.savepath,depth_img)
            od_cammat = cammat2o3d(self.cam_mats[cam_index], self.img_width, self.img_height)
    ...
    def depthimg2Meters(self, depth):
        ...
        image = depth #TODO: Ishaan's update
        return image
    ...
    def captureImage(self, cam_ind, capture_depth=True):
        rendered_images = self.sim.render(self.img_width, self.img_height, camera_id=cam_ind, depth=capture_depth)
        if capture_depth:
