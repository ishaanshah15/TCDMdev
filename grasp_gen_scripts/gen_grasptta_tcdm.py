...
from dataset.utils_HO3D_FPHA import get_diameter
...
def gen_grasps(args, model, cmap_model, eval_loader, device, rh_mano, rh_faces):
    model.eval()
    cmap_model.eval()
    rh_mano.eval()
    tcdm_pc_dir = '/home/ishaans/object_point_clouds_v8'
    save_dir = '/home/ishaans/grasp_tta_output_v10'
    print(len(os.listdir(tcdm_pc_dir)))
    obj_pc_paths = [x for x in os.listdir(tcdm_pc_dir) if 'points' in x]
    print(obj_pc_paths)
    for pc_path in obj_pc_paths:
        full_path = os.path.join(tcdm_pc_dir,pc_path)
        obj_pc = np.load(full_path)
        if len(obj_pc) == 0:
            continue
        trans = np.array([-0.0793, 0.0208, -0.6924]) - np.mean(obj_pc,axis=0)
        obj_pc = obj_pc + trans
        scale = get_diameter(obj_pc)
        #print('scale',scale)
        obj_pc = torch.tensor(obj_pc)
        obj_pc_scaled = torch.concat((obj_pc,torch.tensor([[scale]]*len(obj_pc))),dim=1) #[N,4] 4th column will be scale repeated N times
        obj_pc_scaled = obj_pc_scaled.unsqueeze(dim=0).permute(0,2,1).type(torch.FloatTensor).to(device)
        recon_param = model.inference(obj_pc_scaled).detach() 
        recon_mano = rh_mano(betas=recon_param[:, :10], global_orient=recon_param[:, 10:13],
                                     hand_pose=recon_param[:, 13:58], transl=recon_param[:, 58:], return_full_pose=True)
        recon_xyz = recon_mano.vertices.detach().cpu().numpy()  # [B,778,3], hand vertices

        verts_rh = recon_mano.vertices.detach().cpu().numpy()
        joints_rh = recon_mano.joints.detach().cpu().numpy()
        pose_rh = recon_mano.full_pose.reshape(-1, 16, 3).detach().cpu().numpy()

        

        np.save(os.path.join(save_dir,pc_path.split('.')[0]),recon_xyz)
        np.save(os.path.join(save_dir,pc_path.split('.')[0] + '_object'),np.array(obj_pc_scaled.detach().cpu()))

        

        out_mano_dict = {'vertices':verts_rh, 'joints':joints_rh, 'pose':pose_rh, 
                    'global_orient':recon_mano.global_orient.detach().cpu().numpy(),
                    'betas':recon_mano.betas.detach().cpu().numpy()}
        np.save(os.path.join(save_dir,pc_path.split('.')[0] + '_mano_output'),out_mano_dict)
...
if __name__ == '__main__':
    ...
    #import ipdb
    #ipdb.set_trace()
    gen_grasps(args, affordance_model, cmap_model, dataloader, device, rh_mano, rh_faces)
